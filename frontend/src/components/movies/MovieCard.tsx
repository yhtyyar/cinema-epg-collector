    ...
    <Link to={`/movie/${movie.id}`} className="group rounded-lg overflow-hidden border border-gray-200 dark:border-white/10 hover:shadow-soft transition bg-white dark:bg-white/5">
      <ResponsivePoster src={movie.poster_url} alt={movie.title}>
        ...
        {typeof movie.rating === 'number' ? (
          ...
          <div className="ml-auto">
            <span
              className="pointer-events-auto inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-medium shadow bg-purple-600 text-white"
      ...
      <div className="p-3 space-y-1">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500">
          ...
          {firstCategory && (
            <button
              type="button"
              aria-label={`Показать фильмы категории ${firstCategory}`}
              className="inline-block rounded-full bg-gray-200 hover:bg-gray-300 dark:bg-white/10 dark:hover:bg-white/20 px-2 py-0.5 text-[10px] text-gray-700 dark:text-gray-300"
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); navigate(`/?genre=${encodeURIComponent(firstCategory)}`) }}
            >
              {firstCategory}
            </button>
          )}
