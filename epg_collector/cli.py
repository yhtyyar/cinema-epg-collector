...
from .simple_smart_epg import fetch_epg_smart as fetch_epg
...
def _enrich(limit: Optional[int] = None) -> None:
    ...
    for item in track(movies, description="Обогащение TMDB и загрузка постеров"):
        ...
        if prev:
            ...
            if prev_poster and prev_poster:
                prev_path = Path(prev_poster)
                if is_valid_image_file(prev_path):
                    enriched.append({**item, "tmdb_data": prev.get("tmdb_data"), "poster_local": prev_poster, "poster_source": prev_source})
                    continue

        if not isinstance(title, str) or not title.strip():
            enriched.append({**item, "tmdb_data": None, "poster_local": None})
            continue
        # Получаем данные из TMDB
        info = tmdb.get_movie_info(title)

        # Подбор URL постера: сначала TMDB, затем превью из EPG
        ...
        enriched.append({**item, "tmdb_data": info, "poster_local": poster_local, "poster_source": poster_source})
